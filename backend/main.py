...
if OPENAI_API_KEY:
    ...
    print("WARNING: OPENAI_API_KEY is not set - OpenAI endpoints will be disabled.")
...
def get_klines(symbol: str, interval: str = "1m", limit: int = 200) -> List[List[str]]:
    symbol = symbol.upper()
...
# -------------------------------
# NEW: OpenAI multi-timeframe signal for BUY/HOLD/SELL
# -------------------------------

MULTI_TF_LIST = ["1m", "5m", "15m", "1h", "4h", "1d"]


def build_tf_summary(symbol: str) -> Tuple[str, Dict[str, dict]]:
    """
    Pulls multiple timeframes and returns:
    - A compact text summary for OpenAI
    - A dict of per-TF latest data (for debugging or UI).
    """
    symbol = symbol.upper()
    ...
    for tf in MULTI_TF_LIST:
        ...
        df = df.dropna().reset_index(drop=True)
        if len(df) == 0:
            lines.append(f"{tf}: not enough candles after indicators.")
            continue
    ...
    summary_text = "\n".join(lines)
    return summary_text, tf_data


@app.get("/ai_multi_tf_signal/{symbol}")
def ai_multi_tf_signal(symbol: str):
    """
    MAIN OpenAI brain for your dashboard:
    - Looks at multiple timeframes (1m..1d)
    - Returns a single BUY / HOLD / SELL decision + explanation.

    This is what you should call from the circled 'AI Signal' panel.
    """
    if not OPENAI_API_KEY or client is None:
        return {
            "ok": False,
    ...
    summary, tf_meta = build_tf_summary(symbol)
    if not summary.strip():
        return {
            "ok": False,
            "symbol": symbol.upper(),
            "decision": "HOLD",
            "analysis": "No multi-timeframe data available.",
        }

    system_prompt = """
You are an advanced crypto trading assistant.
You receive a summary of recent multi-timeframe data (1m, 5m, 15m, 1h, 4h, 1d)
for one symbol (like BTCUSDT).

Your job:
- Decide ONE clear action: BUY, SELL, or HOLD.
- This is *not* financial advice; produce a *technical* decision only.
- Be strict but not scared: if the trend and momentum are strong, you can BUY or SELL.
- Use all timeframes together: do not overreact to 1m if higher TFs disagree.
...
def ai_master_signal(symbol: str, interval: str = Query("1h")):
    """
    MAIN SIGNAL: OpenAI analyzes rules data + ML + multi-TF → single BUY/HOLD/SELL
    This drives your dashboard's primary badge.
    """
    if not OPENAI_API_KEY or client is None:
        return {"ok": False, "decision": "HOLD", "reason": "No OpenAI key"}

    # 1. Get rule-based data (RSI/EMA/MACD/BB)
    rules_data = get_klines(symbol.upper(), interval=interval)
    if not rules_data:
        return {"ok": False, "decision": "HOLD", "reason": "No rules data"}

    df_rules = pd.DataFrame(rules_data, columns=["time","o","h","l","c","v","_1","_2","_3","_4","_5","_6"])
    df_rules[["o","h","l","c","v"]] = df_rules[["o","h","l","c","v"]].astype(float)
    df_rules["rsi"] = ta.momentum.RSIIndicator(df_rules["c"]).rsi()
    df_rules["ema20"] = ta.trend.EMAIndicator(df_rules["c"],20).ema_indicator()
    df_rules["ema50"] = ta.trend.EMAIndicator(df_rules["c"],50).ema_indicator()
    last_rules = df_rules.iloc[-1]
    
    # 2. Get multi-TF summary
    tf_summary, _ = build_tf_summary(symbol)
    
    # 3. Get ML prediction
    ml_bundle = ml_bundles.get("15m")  # default
    if ml_bundle:
        ml_pred = predict(symbol, "15m")["prediction"]
    else:
        ml_pred = "HOLD"
    
    prompt = f"""
You are the master trading AI. Analyze this data and give ONE final BUY/HOLD/SELL:

RULES DATA ({interval}): RSI={last_rules.rsi:.1f}, EMA20={last_rules.ema20:.0f}, EMA50={last_rules.ema50:.0f}, Price={last_rules.c:.2f}
...
Respond EXACTLY:
{{"decision": "BUY"|"HOLD"|"SELL", "reason": "1 sentence", "confidence": 65}}
"""
    ...
    try:
        ...
        return {
            ...
            "rules": {"rsi": float(last_rules.rsi), "price": float(last_rules.c)}
        }
    except:
        return {"ok": False, "decision": "HOLD", "reason": "AI error"}
...
"""
    ...
    try:
        ...
        print("[AI_MULTI_TF] OpenAI error:", e)
        return {
            "ok": False,
            "symbol": symbol.upper(),
            "decision": "HOLD",
            "analysis": f"Error talking to OpenAI: {e}",
        }

    # Very small, safe JSON "parser": look for the fields manually
    decision = "HOLD"
    confidence = None
    reason = ""
    notes = ""
    ...
    try:
        # naive parse: model returns proper JSON; if not, we fall back.
        import json
