...
def validate_key(key: str):
    valid_keys = {
        "AADXM": {"valid": True},  # Permanent access
        "AADXM-2025-PRO": {"valid": True, "expires": "2026-12-31"},
        "AADXM-TEST": {"valid": True},  # Permanent test key
        # Add more keys here or connect to database
    }
    result = valid_keys.get(key.upper(), {"valid": False, "message": "Invalid key"})
    return {"valid": result["valid"], "message": result.get("message", ""), "expires": result.get("expires", None)}
